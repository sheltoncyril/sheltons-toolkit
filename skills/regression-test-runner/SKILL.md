...
```bash
oc run log-reader --rm -i --restart=Never -n test-runner --image=quay.io/scyril/busybox:latest --overrides='{"spec":{"volumes":[{"name":"logs","persistentVolumeClaim":{"claimName":"regression-logs"}}],"containers":[{"name":"log-reader","image":"quay.io/scyril/busybox:latest","command":["tail","-100","/logs/<COMPONENT>/<TIER>.log"],"volumeMounts":[{"name":"logs","mountPath":"/logs"}]}]}}' 2>&1
```
...
```bash
oc run log-reader --rm -i --restart=Never -n test-runner --image=quay.io/scyril/busybox:latest --overrides='{"spec":{"volumes":[{"name":"logs","persistentVolumeClaim":{"claimName":"regression-logs"}}],"containers":[{"name":"log-reader","image":"quay.io/scyril/busybox:latest","command":["cat","/logs/<COMPONENT>/<TIER>.log"],"volumeMounts":[{"name":"logs","mountPath":"/logs"}]}]}}' > /tmp/regression-output-<COMPONENT>-<TIER>.log 2>&1
```
...
```bash
oc run log-cleanup --rm -i --restart=Never -n test-runner --image=quay.io/scyril/busybox:latest --overrides='{"spec":{"volumes":[{"name":"logs","persistentVolumeClaim":{"claimName":"regression-logs"}}],"containers":[{"name":"log-cleanup","image":"quay.io/scyril/busybox:latest","command":["rm","-rf","/logs/<COMPONENT>"],"volumeMounts":[{"name":"logs","mountPath":"/logs"}]}]}}'
```
